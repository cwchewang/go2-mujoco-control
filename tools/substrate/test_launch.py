...
class FakePlant:
    ...
    lower, upper = np.full(12, -40.0), np.full(12, 40.0)
    ...
    def snapshot(self, tick):
        qpos = np.zeros(19)
        qpos[:4] = [self.steps * 0.002 * 0.15, 0.0, 0.27, 1.0]
        qpos[7:] = self.steps * 0.0001
        qvel = np.zeros(18)
        qvel[0] = 0.15
        row = dict(
            tick=tick,
            sim_time_s=self.steps * 0.002,
            qpos=qpos.tolist(),
            qvel=qvel.tolist(),
            initial_y=0.0,
            supports=["FR", "FL"],
    ...
    def observe(self):
        ...
        return Proprioception(
            self.names,
            np.array(row["qpos"])[self.qadr],
            np.array(row["qvel"])[self.vadr],
            row["qpos"][3:7],
            row["qvel"][3:6],
        )
...
class LaunchTests(unittest.TestCase):
    def test_streaming_digest_matches_frozen_canonical_json_bytes(self):
        import hashlib

        rows = self.trace()[0]
        for sample in ([], rows[:1], rows):
            filtered = [
                {
                    k: v
                    for k, v in row.items()
                    if k not in ("wall_time_s", "policy_wall_s")
                }
                for row in sample
            ]
            expected = hashlib.sha256(
                json.dumps(
                    filtered, sort_keys=True, allow_nan=False, separators=(",", ":")
                ).encode()
            ).hexdigest()
            self.assertEqual(trace_digest(sample), expected)
    ...
    def setUp(self):
        ...
        self.short["thresholds"]["progress_min_m"] = 0.001
        self.layout = dict(
            names=MOTOR_JOINTS, qadr=list(range(7, 19)), vadr=list(range(6, 18))
        )
    ...
    def analyze(self, rows, protocol=None):
        return analyze(
            rows, protocol or self.short, FakePlant.lower, FakePlant.upper, self.layout
        )

    def test_entire_frozen_horizon_synthetic(self):
        rows, consumed, result, plant, policy = self.trace(self.protocol)
        self.assertEqual(
            (plant.steps, len(rows), len(policy.calls), len(consumed)),
            (5000, 5001, 500, 1),
        )
    ...
    def test_command_boundaries(self):
        self.assertEqual(
            [command_at(self.protocol, t)[0] for t in (0, 499, 500, 750, 1000, 4999)],
            [0, 0, 0, 0.075, 0.15, 0.15],
        )
    ...
    def test_posture_stops_before_next_step(self):
        ...
        self.assertEqual(
            self.analyze(rows)["first_failure"], {"tick": 7, "reason": "posture"}
        )

    def test_initial_failure_never_consumes_attempt(self):
        rows, consumed, _, plant, _ = self.trace(
            fault=lambda row: row.update(warning_count=1)
        )
    ...
    def test_nonfoot_contact_stops(self):
        ...
        self.assertEqual(
            self.analyze(rows)["first_failure"]["reason"], "nonfoot_contact"
        )

    def test_clock_drift_rejected(self):
        with self.assertRaisesRegex(ValueError, "simulation time"):
            self.trace(
                fault=lambda row: row.update(sim_time_s=row["sim_time_s"] + 0.001)
            )
    ...
    def test_wall_timeout_stops(self):
        ...
        self.assertEqual(
            self.analyze(rows, protocol)["first_failure"]["reason"], "wall_timeout"
        )
    ...
    def test_preparation_is_not_start_authorization(self):
        ...
        for value in (
            ...
            with self.assertRaises(ValueError):
                validate_authorization(value, prepared)
        validate_authorization(
            dict(
                action="START_FORMAL_CAPTURE",
                **prepared,
                max_attempts=3,
                authorized_by="user",
                user_instruction="start frozen experiment",
            ),
            prepared,
        )
    ...
    def test_capture_rejects_unapproved_input_before_plant(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("tools.substrate.launch.LOCK_PATH", root / "lock"),
                patch("tools.substrate.launch.verify_bundle", return_value=prepared),
                patch(
                    "tools.substrate.launch.current_identity",
                    return_value=("a" * 40, {}),
                ),
                patch("tools.substrate.launch.MujocoPlant") as plant,
            ):
                with self.assertRaises(ValueError):
                    capture(root, root / "auth.json", root / "capture")
    ...
    def test_independent_watchdog_kills_signal_ignoring_owner(self):
        ...
        result = subprocess.run(
            [sys.executable, "-c", code], capture_output=True, timeout=6
        )
    ...
    def test_repeatability_is_not_inferred_from_three_metric_passes(self):
        rows = self.trace()[0]
        result = analyze(
            rows, self.short, FakePlant.lower, FakePlant.upper, self.layout, "0" * 64
        )
    ...
    def test_zero_step_guard_covers_all_integrators_without_calling_them(self):
        ...
        fake = types.SimpleNamespace(
            mj_step=lambda: None, mj_step1=lambda: None, mj_step2=lambda: None
        )
    ...
    def campaign(self, fault=None):
        ...
        review = dict(
            head="a" * 40,
            science=dict(verdict="APPROVED", reviewer="one", evidence="fixture"),
            execution=dict(verdict="APPROVED", reviewer="two", evidence="fixture"),
        )
        ...
        auth = dict(
            ...
            max_attempts=3,
            authorized_by="user",
            user_instruction="synthetic test fixture only",
        )
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch.object(launch, "FrozenPolicy", return_value=FakePolicy())
            )
    ...
    def test_campaign_three_passes_and_independent_reanalysis(self):
        ...
        self.assertEqual(
            (result["capability_status"], record["live_runs"]), ("PASS", 3)
        )
    ...
    def test_campaign_stops_after_first_failure_without_replacement(self):
        ...
        self.assertEqual(
            [a["status"] for a in verified["attempts"]], ["FAIL", "NOT_RUN", "NOT_RUN"]
        )
    ...
    def test_bad_quaternion_terminal_is_preserved_and_classified(self):
        ...
        self.assertEqual(
            self.analyze(rows)["first_failure"], {"tick": 30, "reason": "orientation"}
        )
