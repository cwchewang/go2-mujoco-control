...
from .integrity import EvidenceRun, LOCK_PATH, digest, strict_json, verify_bundle, write_new, run_logged
...
def validate_review(review, head):
    if not isinstance(review, dict) or set(review) != {"head", "science", "execution"} or review["head"] != head:
        raise ValueError("review must bind exact launch HEAD")
    identities = []
    for role in ("science", "execution"):
        item = review[role]
        if not isinstance(item, dict) or item.get("verdict") != "APPROVED" or not isinstance(item.get("reviewer"), str) or not item["reviewer"].strip() or not isinstance(item.get("evidence"), str) or not item["evidence"].strip():
...
def validate_authorization(value, prepared):
    expected = {
        "action": "START_FORMAL_CAPTURE", "head": prepared["head"],
    ...
    if not isinstance(value, dict) or any(type(value.get(k)) is not type(v) or value[k] != v for k, v in expected.items()):
        raise ValueError("explicit start authorization does not match prepared checkpoint")
    if value.get("authorized_by") != "user" or not isinstance(value.get("user_instruction"), str) or not value["user_instruction"].strip():
        raise ValueError("record the actual user instruction to start; preparation is not authorization")
...
def preflight(lock, head, review, report, fresh_run):
    ...
    argv = [sys.executable, str(ROOT / "tools/research/preflight.py"),
            "--repo-root", str(ROOT), "--experiment-id", "rl-flat-compatibility-v1",
            "--expected-branch", BRANCH, "--expected-head", head,
            "--runner", str(Path(__file__)), "--run-dir", str(fresh_run),
            "--transport", "inprocess", "--held-lock-fd", str(lock.fileno()),
            "--diff-base", "96859ea767efac5557eaf55d95cfd25c95025367",
            "--changed-surface", "runtime", "--changed-surface", "schema",
            "--changed-surface", "runner", "--changed-surface", "analyzer",
            "--requires-sol-review", "--approved-head", head,
            "--test", sys.executable + " -m unittest tools.substrate.test_launch -q",
            "--output", str(report)]
    ...
    run_logged(argv, report.parent, "preflight", timeout=180, cwd=ROOT, pass_fds=(lock.fileno(),), termination_grace=2)
...
def zero_step_guard():
    ...
    originals = {name: getattr(mujoco, name) for name in ("mj_step", "mj_step1", "mj_step2")}
    try:
...
def prepare(directory, review_path):
    with LOCK_PATH.open("a") as lock, zero_step_guard():
        ...
        with EvidenceRun(directory, {"argv": sys.argv, "operation": "prepare_zero_step"}) as run:
            head, sources = current_identity()
            review = strict_json(review_path.read_text())
            preflight(lock, head, review, run.path / "preflight.json", run.path / "future_capture")
            ...
            expected = strict_json((ROOT / "tools/substrate/sources.lock.json").read_text())["rl"]["sha256"]
            ...
            action = policy.act(plant.observe(), [0., 0., 0.]).resolve(plant.observe(), plant.lower, plant.upper, plant.names)
            policy.reset()
            second = policy.act(plant.observe(), [0., 0., 0.]).resolve(plant.observe(), plant.lower, plant.upper, plant.names)
            ...
            write_new(run.path / "initial-action.json", {k: v.tolist() for k, v in action.items()})
            if (head, sources) != current_identity() or closure != dependency_manifest(ROOT / protocol["scene"], ROOT):
                raise ValueError("source changed during prepare")
            run.result.update({
                "status": "ENGINEERING_ADMITTED", "readiness": "READY_AWAITING_START",
                "head": head, "source_files": sources, "runtime": runtime,
                "protocol_sha256": digest(PROTOCOL), "checkpoint_sha256": expected,
                "model": closure, "physical_sha256": physical_fingerprint(plant.model),
                "review": review, "physics_steps": 0, "scientific_attempts": 0,
                "layout": {"names": plant.names, "qadr": plant.qadr, "vadr": plant.vadr},
                "lower": plant.lower.tolist(), "upper": plant.upper.tolist(),
            })
    verify_bundle(directory)
    return {"readiness": "READY_AWAITING_START", "head": head, "physics_steps": 0, "output": str(directory)}
...
def wall_deadline(seconds):
    ...
    guard = subprocess.Popen([sys.executable, str(Path(__file__).with_name("watchdog.py")), str(os.getpid()), str(seconds)], stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
...
def capture(prepared_dir, authorization_path, output):
    ...
    with LOCK_PATH.open("a") as lock:
        ...
        head, sources = current_identity()
        if prepared.get("readiness") != "READY_AWAITING_START" or prepared["head"] != head or prepared["source_files"] != sources or prepared["protocol_sha256"] != digest(PROTOCOL):
        ...
        setup_runtime()
        protocol = strict_json(PROTOCOL.read_text())
        if digest(CHECKPOINT) != prepared["checkpoint_sha256"] or dependency_manifest(ROOT / protocol["scene"], ROOT) != prepared["model"]:
        ...
        if ledger.exists():
            raise ValueError("campaign already claimed; inspect preserved evidence, no autonomous retry")
        with EvidenceRun(output, {"argv": sys.argv, "operation": "formal_capture"}) as run:
            run.result.update({"scope": "formal_exploratory_capture", "capability_status": "INCOMPLETE", "status": "FAILED"})
            preflight(lock, head, prepared["review"], run.path / "preflight.json", ledger)
            ...
            write_new(run.path / "protocol.json", protocol)
            write_new(run.path / "preparation-reference.json", {"path": str(prepared_dir), "manifest_sha256": prepared["prepared_manifest_sha256"]})
            ledger.mkdir(parents=True, exist_ok=False)
            claim = {"head": head, "output": str(run.path.resolve()), "protocol_sha256": prepared["protocol_sha256"]}
            write_new(ledger / "campaign.json", claim)
            write_new(run.path / "campaign-claim.json", claim)
            attempts = [{"index": i, "status": "NOT_RUN"} for i in range(1, protocol["max_attempts"] + 1)]
            ...
            try:
                for item in attempts:
                    ...
                    item["status"] = "BOOTING"
                    def consume():
                        claim = {"index": number, "head": head, "raw": str(raw.resolve()), "boundary": "first_post_handoff_state_control_sample"}
                        write_new(ledger / ("attempt_%02d.json" % number), claim)
                        write_new(run.path / ("attempt_%02d_claim.json" % number), claim)
                    ...
                    reference_trace = attempts[0].get("analysis", {}).get("trace_sha256") if number > 1 else None
                    result = analyze(rows, protocol, plant.lower, plant.upper, prepared["layout"], reference_trace)
                    after = resource.getrusage(resource.RUSAGE_SELF)
                    result["resources"] = {"elapsed_s": time.monotonic() - started, "cpu_user_s": after.ru_utime - usage.ru_utime, "cpu_system_s": after.ru_stime - usage.ru_stime, "process_peak_rss_kib": after.ru_maxrss}
                    write_new(run.path / ("attempt_%02d_analysis.json" % number), result)
                    item.update({"status": result["verdict"], "analysis": result})
                    if (head, sources) != current_identity():
                        raise ValueError("source_changed")
                    if result["verdict"] != "PASS":
                        break
                run.result["capability_status"] = "PASS" if all(v["status"] == "PASS" for v in attempts) else "FAIL"
                ...
                raise
    return {"status": run.result["status"], "capability_status": run.result["capability_status"]}
...
def main():
    ...
    try:
        result = prepare(args.output, args.review) if args.operation == "prepare" else capture(args.prepared, args.authorization, args.output)
        print(json.dumps(result))
        return 0
    except (Exception, KeyboardInterrupt) as exc:
        print(json.dumps({"status": "FAILED", "reason": type(exc).__name__ + ": " + str(exc)}))
        return 1
