...
def _validate_frame(row, tick, frame_count, protocol, initial_y):
    if type(row["tick"]) is not int or row["tick"] != tick:
        raise ValueError("missing or duplicate tick")
    if type(row["sim_time_s"]) not in (int, float) or not math.isclose(
        row["sim_time_s"],
        tick * protocol["physics_period_s"],
        abs_tol=1e-9,
        rel_tol=0,
    ):
    ...
    if not isinstance(row["forbidden_contacts"], list) or not isinstance(
        row["supports"], list
    ):
        raise ValueError("invalid contact evidence")
    if any(x not in ("FR", "FL", "RR", "RL") for x in row["supports"]) or len(
        set(row["supports"])
    ) != len(row["supports"]):
        raise ValueError("invalid support evidence")
    if row["terminal_reason"] and tick != frame_count - 1:
    ...
    return qpos, qvel, reason


def _validate_action(row, qpos, qvel, lower, upper, layout):
    action = row["action"]
    ff, pd, total, ctrl = [
        vector(action[k], 12, k)
        for k in ("feedforward", "pd", "total_unclipped", "ctrl")
    ]
    if not np.array_equal(ff + pd, total) or not np.array_equal(
        np.clip(total, lower, upper), ctrl
    ):
    ...
    if (
        ...
        or not np.array_equal(mask, (total < lower) | (total > upper))
    ):
    ...
    return int(mask.sum()), target


def trace_digest(rows):
    """Preserve the v1 canonical JSON hash with only one row-sized scratch buffer."""
    digest = hashlib.sha256(b"[")
    for index, row in enumerate(rows):
        if index:
            digest.update(b",")
        value = {
            k: v for k, v in row.items() if k not in ("wall_time_s", "policy_wall_s")
        }
        digest.update(
            json.dumps(
...
def analyze(rows, protocol, lower, upper, layout, reference_trace_sha256=None):
    ...
    for tick, row in enumerate(rows):
        qpos, qvel, reason = _validate_frame(row, tick, len(rows), protocol, initial_y)
        ...
        if row["action"] is not None:
            ...
            count, target = _validate_action(row, qpos, qvel, lower, upper, layout)
            saturated += count
            if tick % decimation:
    ...
    last = rows[-1]
    if not last["terminal_reason"] or last["terminal_reason"] != (
        last["failure"] or "horizon"
    ):
        raise ValueError("incomplete or inconsistent terminal evidence")
    if (
        last["terminal_reason"] == "horizon"
        and last["tick"] != protocol["horizon_ticks"]
    ):
        raise ValueError("premature horizon")
    progress = last["qpos"][0] - initial_x
    mae = float(np.mean(errors)) if errors else None
    signature = trace_digest(rows)
    repeatable = reference_trace_sha256 is None or signature == reference_trace_sha256
    passed = (
        repeatable
        and first_failure is None
        and last["tick"] == protocol["horizon_ticks"]
        and progress >= protocol["thresholds"]["progress_min_m"]
        and mae is not None
        and mae <= protocol["thresholds"]["vx_mae_max_mps"]
    )
    return {
        "verdict": "PASS" if passed else "FAIL",
        "first_failure": first_failure,
        "trace_sha256": signature,
        "repeatability_matches": repeatable,
        ...
        "policy_wall_p95_s": float(np.quantile(latencies, 0.95)) if latencies else None,
        "wall_time_s": last["wall_time_s"],
        "real_time_factor": last["sim_time_s"] / last["wall_time_s"]
        if last["wall_time_s"] > 0
        else None,
    }
