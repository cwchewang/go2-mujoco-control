...
{
    // Default-off Phase1 RR-thigh D ablation. The gate uses the same
    // active-relative gait clock and period as the command profile; all
    // controller outputs stay unchanged and only motor 7 kd is attenuated.
    rr_thigh_d_gate_time_s_ = gait_elapsed_s;
    const double gate_period_s = std::max(params_.period_s, 1.0e-6);
    rr_thigh_d_gate_phase_ = std::fmod(
        std::max(0.0, gait_elapsed_s) / gate_period_s, 1.0);
    if (rr_thigh_d_gate_phase_ < 0.0)
        rr_thigh_d_gate_phase_ += 1.0;
    rr_thigh_d_gate_active_ =
        rr_thigh_d_gated_ab_enabled_ &&
        gait_elapsed_s >= 32.10 && gait_elapsed_s < 32.60 &&
        rr_thigh_d_gate_phase_ >= 0.50 &&
        rr_thigh_d_gate_phase_ < 0.75;
    rr_thigh_d_kd_scale_ = rr_thigh_d_gate_active_ ? 0.75 : 1.0;
    rr_thigh_d_kd_baseline_ = 0.0;
    rr_thigh_d_kd_effective_ = 0.0;
    ...
    for (std::size_t leg = 0; leg < go2::kLegCount; ++leg)
    {
        ...
        for (int joint = 0; joint < 3; ++joint)
        {
            ...
            if (i == 7)
            {
                rr_thigh_d_kd_baseline_ = cmd_kd;
                cmd_kd *= rr_thigh_d_kd_scale_;
                rr_thigh_d_kd_effective_ = cmd_kd;
            }
    ...
    for (int i = 0; i < kMotorCount; ++i)
    {
        ...
        double cmd_kd = task_.motion_stage_ == 0 ? 3.5 : params_.kd;
        if (i == 7)
        {
            rr_thigh_d_kd_baseline_ = cmd_kd;
            cmd_kd *= rr_thigh_d_kd_scale_;
