...
def make_script(
    path: Path, source: str, target: str, source_mag: float, target_mag: float
) -> None:
...
def obstacle_metrics(
    run_dir: Path, uses_obstacle: bool
    ...
    force = max(
        float(row.get("reactive_obstacle_contact_force_N", "0")) for row in rows
    )
...
def write_index(path: Path, records: list[dict[str, object]]) -> None:
    fields = [
        ...
        "video",
        "duration_s",
        "obstacle_contact_force_max_N",
        "obstacle_contact_count_max",
        "obstacle_contact_pass",
    ]
...
def build_sequential(
    ffmpeg: Path, output_root: Path, records: list[dict[str, object]]
) -> None:
...
def main() -> int:
    ...
    parser.add_argument(
        "--physical-obstacle",
        action="store_true",
        help="Use the collision-box scene; the default matrix isolates reference handoffs on the nominal floor.",
    )
    ...
    if len(selected) != args.count:
        raise SystemExit(
            f"expected {args.count} source manifests, found {len(selected)}"
        )
    ...
    for ordinal, source_item in enumerate(selected):
        ...
        uses_obstacle = args.physical_obstacle and (
            "obstacle" in source or "obstacle" in target
        )
        ...
        fallback = [
            (
                2.5,
                6.5,
                f"A: {source.replace('_', ' ').upper()}",
                "green" if "obstacle" in source else "orange",
            ),
            (
                6.5,
                10.5,
                f"B: {target.replace('_', ' ').upper()}",
                "green" if "obstacle" in target else "cyan",
            ),
        ]
        spec = {
            "title": f"REACTIVE TRANSITION {index:03d} | {source.replace('_', ' ').upper()} -> {target.replace('_', ' ').upper()}",
            "subtitle": "A and B: 4.0 s each | continuous reference handoff | recovery",
            "clip_start": args.clip_start,
            "clip_duration": args.clip_duration,
        }
        item: dict[str, object] = {
            "protocol_name": "go2_reactive_transition_matrix_long",
            "protocol_version": "1.0",
            "pair_index": index,
            "run_id": run_id,
            "source_event": source,
            "target_event": target,
            "event_script": str(event_script.relative_to(protocol)),
            "run_directory": str(run_dir),
            "video": str(final.relative_to(args.output_root)),
            "scene_file": OBSTACLE_SCENE
            if uses_obstacle
            else "unitree_robots/go2/scene_leg_lift_demo.xml",
            "status": "pending",
        }
        if (
            final.exists()
            and final.stat().st_size >= 100_000
            and run_dir.joinpath("data.csv").exists()
        ):
            ...
            item.update(
                {
                    ...
                    "obstacle_contact_force_max_N": force,
                    "obstacle_contact_count_max": count,
                    "obstacle_contact_pass": passed,
                }
            )
            records.append(item)
            continue
        ...
        if result.returncode != 0:
            manifest_path.write_text(
                json.dumps(
                    sorted(records, key=lambda entry: int(entry["pair_index"])),
                    ensure_ascii=False,
                    indent=2,
                ),
                encoding="utf-8",
    ...
    return (
        0
        if args.dry_run or all(item.get("status") == "pass" for item in records)
        else 1
    )
