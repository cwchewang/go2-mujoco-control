...
SPECS = [
    {
        "id": "impact_strong_recovery",
        "title": "IMPACT | PHYSICAL PUSH -> AUTO EMERGENCY STOP",
        "subtitle": "0.80 m/s MuJoCo push | detector -> 0.50 s transition -> WBC stance hold",
        "controller_duration": 16.0,
        "raw_duration": 17.0,
        "clip_start": 2.0,
        "clip_duration": 15.0,
        "fallback": [
            (4.7, 5.2, "PHYSICAL IMPACT", "red"),
            (5.2, 7.5, "EMERGENCY STOP", "red"),
        ],
        "args": [
            "--reactive-events",
            "--impact-to-emergency-stop-delay",
            "0.5",
            "--push-time",
            "8.0",
            "--push-vel-x",
    ...
    {
        "id": "slip_reference",
        "title": "SLIP | reference protection",
        "subtitle": "scripted velocity-mismatch response; no floor parameter change",
        "controller_duration": 16.0,
        "raw_duration": 17.0,
        "clip_start": 3.5,
        "clip_duration": 13.0,
        "event_script": "slip_reference.txt",
        "fallback": [(2.5, 6.5, "SLIP RESPONSE", "orange")],
        "args": [],
    },
    {
        "id": "low_friction_physical",
        "title": "LOW FRICTION | physical floor change",
        "subtitle": "MuJoCo floor friction mu=0.02 for 4.0 s",
        "controller_duration": 16.0,
        "raw_duration": 17.0,
        "clip_start": 3.5,
        "clip_duration": 13.0,
        "fallback": [(4.0, 8.0, "FLOOR MU = 0.02", "blue")],
        "args": [
            "--reactive-events",
            "--friction-time",
            "8.0",
    ...
    {
        "id": "turn_left_long",
        "title": "TURN LEFT | yaw-only command",
        "subtitle": "continuous reference: yaw rate only, forward speed retained",
        "controller_duration": 16.0,
        "raw_duration": 17.0,
        "clip_start": 3.5,
        "clip_duration": 13.0,
        "event_script": "turn_left_long.txt",
        "fallback": [(2.5, 6.5, "TURN LEFT", "cyan")],
        "args": [],
    },
    {
        "id": "obstacle_left_physical",
        "title": "OBSTACLE LEFT | physical box avoidance",
        "subtitle": "real collision geom; lane change + bounded yaw, zero contact required",
        "controller_duration": 18.0,
        "raw_duration": 19.0,
        "clip_start": 3.5,
        "clip_duration": 15.0,
        "event_script": "obstacle_left_physical.txt",
        "scene_file": "unitree_robots/go2/scene_reactive_obstacle.xml",
        "fallback": [(2.5, 10.5, "OBSTACLE LEFT / LANE CHANGE", "green")],
        "args": [],
    },
    {
        "id": "obstacle_to_turn_handoff",
        "title": "HANDOFF | obstacle-left -> turn-left",
        "subtitle": "4.0 s obstacle response, 4.0 s turn response, then recovery",
        "controller_duration": 18.0,
        "raw_duration": 19.0,
        "clip_start": 3.5,
        "clip_duration": 15.0,
        "event_script": "obstacle_to_turn_handoff.txt",
        "scene_file": "unitree_robots/go2/scene_reactive_obstacle.xml",
        "fallback": [
            (2.5, 6.5, "OBSTACLE LEFT", "green"),
            (6.5, 10.5, "TURN LEFT", "cyan"),
        ],
        "args": [],
    },
]
...
def event_spans(
    run_dir: Path, clip_start: float, fallback: list[tuple[float, float, str, str]]
    ...
    for row in rows + [
        {
            "cmd_time_s": rows[-1].get("cmd_time_s", str(started))
            if rows
            else str(started),
            "event_type": "0",
        }
        ...
        if previous != "none" and now > started:
            spans.append(
                (
                    max(0.0, started - clip_start),
                    max(0.05, now - clip_start),
                    previous.replace("_", " ").upper(),
                    color_for(previous),
                )
            )
...
def render(
    ...
    for start, end, label, color in spans:
        ...
        filters.append(
            f"drawbox=x=0:y=ih-34:w=iw:h=34:color={colors.get(color, 'orange@0.70')}:t=fill:enable='{enable}'"
        )
        filters.append(
            f"drawtext=fontfile={font}:text='{esc(label)}':x=w/2-125:y=h-27:fontsize=18:fontcolor=white:enable='{enable}'"
        )
    command = [
        str(ffmpeg),
        "-y",
...
def main() -> int:
    ...
    for ordinal, spec in enumerate(SPECS):
        ...
        if spec.get("event_script"):
            script = (
                repo
                / "example/cpp/experiments/go2_reactive_representatives_2026-08-20/event_scripts"
                / str(spec["event_script"])
            )
        ...
        item: dict[str, object] = {
            ...
            "event_script": str(script) if script else None,
            "clip_start_s": spec["clip_start"],
            "clip_duration_s": spec["clip_duration"],
            "status": "pending",
        }
        if final.exists() and final.stat().st_size >= 100_000:
            spans = event_spans(
                run_dir, float(spec["clip_start"]), list(spec["fallback"])
            )
            item.update(
                {
                    "status": "pass",
                    ...
                    "resumed": True,
                    "event_spans": [
                        {"start_s": s, "end_s": e, "label": label, "color": color}
        ...
        env.update(
            {
                "TROT_RECORD_DURATION_S": str(spec["raw_duration"]),
                "TROT_RECORD_FPS": "20",
        ...
        command = [
            ...
            "35",
            "--controller-duration",
            str(spec["controller_duration"]),
            "--domain-id",
            str(args.domain_base + ordinal),
            "--camera-follow",
        ]
        if script:
