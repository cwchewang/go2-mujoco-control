...
def sustained_surface_contact(rows, leg, box, deadline_s):
    ...
    for row in rows:
        ...
        inside = (
            time_s <= deadline_s
            and box["x_min_m"] + 0.01 <= x <= box["x_max_m"] - 0.01
            and box["y_min_m"] + 0.03 <= y <= box["y_max_m"] - 0.03
            and abs(z - expected_z) <= 0.030
            and force >= 5.0
        )
        if inside:
...
def first_clear_time(rows, final_box, deadline_s):
    ...
    for row in rows:
        ...
        if time_s > deadline_s:
            break
        if (
            number(row, "base_pos_world_x_m") >= clear_x
            and number(row, "base_pos_world_z_m") >= 0.28
            and all(number(row, f"{leg}_pos_world_x_m") >= foot_clear_x for leg in LEGS)
        ):
            return time_s
    return None
...
def main():
    ...
    for leg in LEGS:
        truth_required |= {
            f"{leg}_pos_world_x_m",
            f"{leg}_pos_world_y_m",
            f"{leg}_pos_world_z_m",
            f"{leg}_foot_contact_grf_world_z_N",
        }
    truth_missing = sorted(truth_required - set(truth[0]))
    if truth_missing:
        raise SystemExit(
            "missing terrain ground-truth columns: " + ", ".join(truth_missing)
        )
    ...
    required = {
        ...
        "velocity_command_accel_mps2",
        "velocity_command_jerk_mps3",
        "imu_roll_rad",
        "imu_pitch_rad",
        "world_base_z_m",
        "terrain_map_source",
        ...
        "terrain_solver_elapsed_us",
        "terrain_safe_stop_requested",
        ...
        "terrain_target_prepare_attempts",
        "terrain_target_prepared",
        "terrain_target_prepare_rejections",
        "wbc_measured_contact_mask",
        "wbc_scheduled_contact_mask",
        "wbc_terrain_planned_contact_mask",
        "wbc_mpc_update_count",
        "wbc_mpc_contact_mask_k0",
        "wbc_mpc_min_contact_count",
        "wbc_mpc_reference_x_first_m",
        "wbc_mpc_reference_x_last_m",
        "wbc_mpc_reference_vx_first_mps",
        "wbc_mpc_reference_vx_last_mps",
        "wbc_terrain_contact_coherent",
        "wbc_terrain_plan_id",
        "kernel_footstep_plan_valid",
        "wbc_full_srbd_ok",
        "wbc_full_id_ok",
        "wbc_shadow_within_budget",
        "support_low_friction_evidence",
        "support_foot_speed_mps",
        "contact_count",
        "touchdown_x_error_m",
        "touchdown_y_error_m",
        "wbc_shadow_max_abs_tau",
    }
    for leg in LEGS:
        required |= {
            f"terrain_exec_{leg}_valid",
            f"terrain_exec_{leg}_in_flight",
    ...
    active = [
        row
        for row in data
        if number(row, "motion_stage") == 2
        and number(row, "velocity_command_active") > 0.5
    ]
    if not active:
    ...
    failure_ticks = [
        number(row, "state_tick_s")
        for row in active
        if number(row, "terrain_safe_stop_requested") > 0.5
        or abs(math.degrees(number(row, "imu_roll_rad"))) > 15.0
        or abs(math.degrees(number(row, "imu_pitch_rad"))) > 15.0
        or number(row, "world_base_z_m") < 0.28
    ]
    evidence_deadline_s = min(failure_ticks, default=last_tick)
    safe_active = [
        row for row in active if number(row, "state_tick_s") <= evidence_deadline_s
    ]
    ...
    valid_plans = [
        row
        for row in active
        if number(row, "terrain_plan_status") == 1
        and number(row, "terrain_plan_valid") > 0.5
    ]
    execution_rows = [
        row
        for row in active
        if any(
            number(row, f"terrain_exec_{leg}_valid") > 0.5
            and number(row, f"terrain_exec_{leg}_target_required") > 0.5
            for leg in LEGS
        )
    ]
    coherent_rows = [
        row for row in execution_rows if number(row, "wbc_terrain_plan_id") > 0
    ]
    transfer_mpc_rows = []
    ...
    for row in execution_rows:
        ...
        if (
            number(row, "terrain_transfer_hold_active") > 0.5
            or number(row, "terrain_surface_transition_active") > 0.5
        ) and update > previous_mpc_update:
            transfer_mpc_rows.append(row)
        previous_mpc_update = max(previous_mpc_update, update)
    required_rejection_rows = [
        row
        for row in active
        if number(row, "terrain_plan_status") == 4
        and any(number(row, f"terrain_{leg}_candidate_required") > 0.5 for leg in LEGS)
    ]

    tracking_error = [
        abs(
            number(row, "velocity_command_shaped_mps")
    ...
    torque_keys = [
        key for key in data[0] if key.endswith("_tau_ff") or key.endswith("_tau_est")
    ]
    torque_values = [abs(number(row, key)) for row in active for key in torque_keys]
    touchdown_rows = [
        row for row in active if number(row, "touchdown_event_count", 0) > 0
    ]
    ...
    for row in execution_rows:
        for leg in LEGS:
            ...
            error = math.sqrt(
                sum(
                    (
                        number(row, f"terrain_exec_{leg}_foot_world_{axis}_m")
                        - number(row, f"terrain_exec_{leg}_target_world_{axis}_m")
                    )
                    ** 2
                    for axis in ("x", "y", "z")
                )
    ...
    for box in boxes:
        leg_results = {
            leg: sustained_surface_contact(truth, leg, box, evidence_deadline_s)
            for leg in LEGS
        }
        surface_results.append(
            {
                **box,
                "legs": leg_results,
                "all_legs_supported": all(
                    item["pass"] for item in leg_results.values()
                ),
            }
        )
    ...
    exit_truth = [
        row
        for row in truth
        if clear_time_s is not None
        and clear_time_s <= number(row, "time_s") <= clear_time_s + 0.50
    ]
    ...
    metrics = {
        ...
        "acceleration_abs_max_mps2": max(
            abs(number(row, "velocity_command_accel_mps2")) for row in active
        ),
        "jerk_abs_max_mps3": max(
            abs(number(row, "velocity_command_jerk_mps3")) for row in active
        ),
        "contact_loss_fraction": sum(
            number(row, "contact_count") <= 0 for row in active
        )
        / len(active),
        "single_contact_fraction": sum(
            number(row, "contact_count") <= 1 for row in active
        )
        / len(active),
        "slip_evidence_rows": sum(
            number(row, "support_low_friction_evidence") > 0 for row in active
        ),
        "torque_saturation_fraction": sum(value >= 45.0 for value in torque_values)
        / len(torque_values),
        "touchdown_x_abs_max_m": max(
            (abs(number(row, "touchdown_x_error_m")) for row in touchdown_rows),
            default=0.0,
        ),
        "touchdown_y_abs_max_m": max(
            (abs(number(row, "touchdown_y_error_m")) for row in touchdown_rows),
            default=0.0,
        ),
        "terrain_touchdown_3d_error_max_m": max(
            target_touchdown_errors, default=float("nan")
        ),
        "planner_latency_p95_us": percentile(planner_samples, 0.95),
        "planner_latency_max_us": max(planner_samples, default=float("nan")),
        "planner_deadline_misses": max(
            number(row, "terrain_planner_deadline_misses") for row in data
        ),
        ...
        "gait_target_overrides": max(
            number(row, "terrain_gait_target_overrides") for row in data
        ),
        "mpc_plan_consumed": max(
            number(row, "terrain_mpc_plan_consumed") for row in data
        ),
        "target_prepare_attempts": max(
            number(row, "terrain_target_prepare_attempts") for row in data
        ),
        "target_prepared": max(number(row, "terrain_target_prepared") for row in data),
        "target_prepare_rejections": max(
            number(row, "terrain_target_prepare_rejections") for row in data
        ),
        "surface_transition_completions": max(
            number(row, "terrain_surface_transition_completions") for row in data
        ),
        "surface_transition_last_required_mask": int(
            max(
                number(row, "terrain_surface_transition_last_required_mask")
                for row in data
            )
        ),
        "surface_transition_last_committed_mask": int(
            max(
                number(row, "terrain_surface_transition_last_committed_mask")
                for row in data
            )
        ),
        "surface_transition_max_committed_contacts": max(
            mask_population(number(row, "terrain_surface_transition_committed_mask"))
            for row in data
        ),
        ...
        "wbc_plan_coherence_fraction": (
            sum(
                number(row, "wbc_terrain_contact_coherent") > 0.5
        ...
        "transfer_mpc_min_contacts": min(
            (number(row, "wbc_mpc_min_contact_count") for row in transfer_mpc_rows),
            default=float("nan"),
        ),
        ...
        "mpc_vcmd_authority_abs_max_mps": max(
            (
                max(
                    abs(
                        number(row, "wbc_mpc_reference_vx_first_mps")
                        - number(row, "velocity_command_applied_mps")
                    ),
                    abs(
                        number(row, "wbc_mpc_reference_vx_last_mps")
                        - number(row, "velocity_command_applied_mps")
                    ),
                )
                for row in transfer_mpc_rows
        ...
        "mpc_horizontal_reference_span_abs_max_m": max(
            (
                abs(
        ...
        "exit_window_s": (
            number(exit_truth[-1], "time_s") - number(exit_truth[0], "time_s")
        )
        if len(exit_truth) >= 2
        else 0.0,
        "ground_truth_collision_rows": sum(
            number(row, "phase2_terrain_nonfoot_contact_count") > 0
            and number(row, "time_s") <= evidence_deadline_s
            for row in truth
        ),
    }

    plan_edge = [number(row, "terrain_min_edge_margin_m") for row in valid_plans]
    plan_uncertainty_edge = [
        number(row, "terrain_min_uncertainty_edge_margin_m") for row in valid_plans
    ]
    ...
    plan_reach = [
        number(row, "terrain_min_reachability_margin_m") for row in valid_plans
    ]
    plan_clearance = [
        number(row, "terrain_min_swing_clearance_m") for row in valid_plans
    ]
    plan_support = [number(row, "terrain_min_support_margin_m") for row in valid_plans]
    plan_uncertainty_support = [
        number(row, "terrain_min_uncertainty_support_margin_m") for row in valid_plans
    ]
    ...
    checks = {
        ...
        "phase1_steps_quantitative": phase1.get("quantitative_scenario") == "steps"
        and phase1.get("quantitative_pass") is True
        and phase1.get("acceptance_status") == "PASS",
        "lidar_only": all(
            row.get("terrain_map_source") in ("lidar", "none") for row in active
        )
        and any(row.get("terrain_map_source") == "lidar" for row in active),
        "no_safe_stop": all(
            number(row, "terrain_safe_stop_requested") < 0.5 for row in active
        ),
        "posture_p95": metrics["roll_abs_p95_deg"] <= 4.0
        and metrics["pitch_abs_p95_deg"] <= 4.0,
        "posture_hard": metrics["roll_abs_max_deg"] <= 15.0
        and metrics["pitch_abs_max_deg"] <= 15.0,
        ...
        "touchdown_xy": metrics["touchdown_x_abs_max_m"] <= 0.18
        and metrics["touchdown_y_abs_max_m"] <= 0.07,
        "terrain_touchdown_error": bool(target_touchdown_errors)
        and metrics["terrain_touchdown_3d_error_max_m"] <= 0.075,
        "torque_saturation": metrics["torque_saturation_fraction"] <= 0.003,
        "no_slip_evidence": metrics["slip_evidence_rows"] == 0,
        "model_validity": all(
            number(row, "kernel_footstep_plan_valid") > 0.5
            and number(row, "wbc_full_srbd_ok") > 0.5
            and number(row, "wbc_full_id_ok") > 0.5
            for row in safe_active
        ),
        "solver_budget": sum(
            number(row, "wbc_shadow_within_budget") > 0.5 for row in safe_active
        )
        / len(safe_active)
        >= 0.80,
        "planner_latency": metrics["planner_latency_p95_us"] <= 2000.0
        and metrics["planner_latency_max_us"] <= 5000.0
        and metrics["planner_deadline_misses"] == 0,
        "required_plan_rejections": metrics["required_plan_rejection_rows"] == 0,
        "plan_pipeline": metrics["plan_published"] > 0
        and metrics["plan_consumed"] > 0
        and metrics["gait_target_overrides"] > 0
        and metrics["mpc_plan_consumed"] > 0
        and metrics["target_prepared"] > 0,
        "surface_transition_transaction": metrics["surface_transition_completions"] > 0
        and metrics["surface_transition_last_required_mask"] > 0
        and metrics["surface_transition_last_required_mask"]
        == metrics["surface_transition_last_committed_mask"],
        "plan_geometry": bool(valid_plans)
        and min(plan_edge) >= 0.040
        and min(plan_uncertainty_edge) >= 0.0
        and max(plan_slope) <= math.radians(20.0)
        and max(plan_roughness) <= 0.025
        and min(plan_reach) >= 0.010
        and min(plan_clearance) >= 0.0,
        "plan_support": bool(valid_plans)
        and min(plan_support) >= 0.015
        and min(plan_uncertainty_support) >= 0.0
        and all(
            mask_population(number(row, "terrain_execution_planned_contact_mask")) != 1
            for row in execution_rows
        ),
        "wbc_plan_coherence": bool(coherent_rows)
        and len(coherent_rows) == len(execution_rows)
        and metrics["wbc_plan_coherence_fraction"] == 1.0,
        "transfer_mpc_support": metrics["transfer_mpc_samples"] > 0
        and metrics["transfer_mpc_min_contacts"] >= 2
        and metrics["transfer_mpc_k0_contact_mismatches"] == 0,
        "single_vcmd_authority": metrics["transfer_mpc_samples"] > 0
        and metrics["mpc_vcmd_authority_abs_max_mps"] <= 0.020
        and metrics["mpc_horizontal_reference_span_abs_max_m"] <= 0.001,
        "ground_truth_surface_support": all(
            item["all_legs_supported"] for item in surface_results
        ),
        ...
        "b3_plan_replacement": args.milestone != "B3"
        or (
            metrics["execution_plan_ids"] >= 2 and metrics["execution_map_epochs"] >= 2
        ),
    }
