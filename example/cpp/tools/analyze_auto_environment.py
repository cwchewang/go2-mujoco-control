...
def contact_max(path: Path):
    ...
    counts, forces = (
        values("reactive_obstacle_contact_count"),
        values("reactive_obstacle_contact_normal_force_N"),
    )
    return {
        "rows": len(data),
        "max_count": max(counts, default=math.nan),
...
def has_obstacle_scene(path: Path) -> bool:
    ...
    if not scene.is_absolute():
        ...
        scene = next(
            (candidate for candidate in candidates if candidate.exists()), scene
        )
    if not scene.exists():
        return False
    try:
        return (
            ET.parse(scene).getroot().find(".//geom[@name='reactive_obstacle']")
            is not None
        )
...
def analyze(path: Path, expect_obstacle: bool):
    ...
    max_age = max(
        (number(row, "environment_map_age_s") for row in valid_map), default=math.nan
    )
    observed = transitions(data)
    non_none = [item for item in observed if item["type"] != "none"]
    obstacle = next(
        (item for item in non_none if str(item["type"]).startswith("obstacle_")), None
    )
    obstacle_events = [
    ...
    if obstacle is not None and math.isfinite(gait_start):
        ...
        next_transition = next(
            (item for item in observed if item["time_s"] > event_time), None
        )
        event_end = (
            float(next_transition["time_s"]) if next_transition else event_time + 8.0
        )
        active = [
            row for row in data if event_time <= number(row, "cmd_time_s") <= event_end
        ]
        before = [
            row
            for row in data
            if event_time - 0.8 <= number(row, "cmd_time_s") < event_time
        ]
        target_window = active[:500]
        vy, yaw, kind = (
            median(target_window, "event_target_vy_mps"),
            median(target_window, "event_target_yaw_rate_radps"),
            str(obstacle["type"]),
        )
        target_ok = (kind == "obstacle_left" and vy >= 0.20 and yaw >= 0.10) or (
            kind == "obstacle_right" and vy <= -0.20 and yaw <= -0.10
        )
        ...
        response_ok = target_ok and (
            (kind == "obstacle_left" and lateral_shift >= 0.05)
            or (kind == "obstacle_right" and lateral_shift <= -0.05)
        )
    ...
    contact_ok = not expect_obstacle or (
        contact["rows"] > 100
    ...
    if expect_obstacle:
        unexpected = [
            item for item in non_none if not str(item["type"]).startswith("obstacle_")
        ]
        gaps_ok = all(
            b["time_s"] - a["time_s"] >= 0.25
    ...
    result.update(
        {
            ...
            "gait_start_s": gait_start,
            "first_map_s": first_map,
            "map_valid_rows": len(valid_map),
            "map_valid_rate_after_first": map_rate,
            "max_map_age_s": max_age,
            "obstacle_event": obstacle,
            "obstacle_events": obstacle_events,
            "detection_latency_after_warmup_s": latency,
...
def main() -> int:
    ...
    reports = [
        analyze(path.resolve(), path.resolve() in obstacle_paths)
        for path in args.experiment
    ]
    summary = {
        "protocol": "automatic environment sensing acceptance v1.0",
        "reports": reports,
    ...
    lines = [
        "# Automatic environment sensing acceptance",
        "",
    ...
    print(
        json.dumps({"strict_pass": summary["strict_pass"], "experiments": len(reports)})
    )
