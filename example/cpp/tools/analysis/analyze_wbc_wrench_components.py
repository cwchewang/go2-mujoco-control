...
def main():
    ...
    torque_columns = [
        name for name in state_rows[common[0]] if name.endswith("_tau_ff_candidate")
    ]
    ...
    active_contacts = []
    for key in common:
        gravity_vector = [float(gravity_rows[key][name]) for name in torque_columns]
        state_vector = [float(state_rows[key][name]) for name in torque_columns]
        force = float(state_rows[key]["desired_force_x_n"])
        if not all(
            math.isfinite(value) for value in gravity_vector + state_vector + [force]
        ):
        ...
        delta.append(
            [
                ...
                for state_value, gravity_value in zip(state_vector, gravity_vector)
            ]
        )
        desired_force.append(force)
        active_contacts.append(int(float(state_rows[key]["selected_contact_count"])))

    nonzero = [
        index
        for index, force in enumerate(desired_force)
        if abs(force) > args.force_epsilon
    ]
    delta_abs = [max(abs(value) for value in delta[index]) for index in nonzero]
    lines = [
    ...
    for contact_count in (2, 4):
        subset_positions = [
            position
            for position, index in enumerate(nonzero)
    ...
    for column_index, column in enumerate(torque_columns):
        ...
        correlations.append(
            (abs(pearson(force_values, values)), column, pearson(force_values, values))
        )
    correlations.sort(reverse=True)
    for _, column, correlation in correlations[:3]:
        lines.append(
            "strongest_delta_force_correlation_%s=%.6f" % (column, correlation)
        )
