...
def main():
    ...
    if missing:
        result = {"experiment": "phase2_known_step_v2_same_binary_ac_20260915", "classification": "PROTOCOL_FAILURE", "missing_runs": missing, "live_process_launched_by_analyzer": False}
        (out / "analysis.json").write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
        (out / "RESULTS.md").write_text("# Same-binary V2 A/C closeout\n\nPrimary classification: `PROTOCOL_FAILURE`\n\nMissing raw run: " + json.dumps(missing) + "\n", encoding="utf-8")
        V2.write_csv(out / "provenance.csv", [])
        print("classification=PROTOCOL_FAILURE missing_runs=" + ",".join(missing)); return 0
