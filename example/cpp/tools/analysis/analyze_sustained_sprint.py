...
def intervals(
    rows: list[dict[str, str]], speed: float, angle: float
    ...
    for row in rows:
        ...
        a = (
            max(abs(float(row["imu_roll_rad"])), abs(float(row["imu_pitch_rad"])))
            * 180.0
            / math.pi
        )
...
def inspect(
    run: Path, speed: float, angle: float, min_window: float
) -> dict[str, object]:
    ...
    angles = [
        max(abs(float(r["imu_roll_rad"])), abs(float(r["imu_pitch_rad"])))
        * 180.0
        / math.pi
        for r in source
    ]
    ...
    final_angle = (
        max(abs(float(final["imu_roll_rad"])), abs(float(final["imu_pitch_rad"])))
        * 180.0
        / math.pi
    )
    ...
    passed = (
        bool(rows)
        and bool(stage3)
        and max(speeds, default=0.0) >= speed
        and window_s >= min_window
        and window_p95 <= angle
        and len(stage3) >= 500
        and final_speed <= 0.15
        and final_angle <= angle
        and controlled_stop
        and safe
        and all(s == "0" for s in statuses)
    )
    return {
        "run": run.name,
...
def main() -> int:
    ...
    results = [
        inspect(
            Path(path), args.speed_threshold, args.angle_limit_deg, args.min_window_s
        )
        for path in args.run_dirs
    ]
    print(
        "run,max_speed_mps,median_speed_mps,p05_speed_mps,p95_speed_mps,good_window_s,window_start_s,window_end_s,window_angle_p95_deg,whole_angle_p95_deg,stage2_rows,stage3_rows,final_speed_mps,final_angle_deg,controlled_stop,safe,statuses,pass"
    )
    for r in results:
        print(
            f"{r['run']},{r['max_speed']:.3f},{r['median_speed']:.3f},{r['p05_speed']:.3f},{r['p95_speed']:.3f},{r['window_s']:.3f},{r['window_start']:.3f},{r['window_end']:.3f},{r['window_angle_p95']:.3f},{r['whole_angle_p95']:.3f},{r['stage2_rows']},{r['stage3_rows']},{r['final_speed']:.3f},{r['final_angle']:.3f},{int(r['controlled_stop'])},{int(r['safe'])},{r['statuses']},{int(r['pass'])}"
        )
