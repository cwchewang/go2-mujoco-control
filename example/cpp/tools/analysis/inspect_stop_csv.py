...
def main() -> None:
    ...
    first_stage3 = next(
        (index for index, row in enumerate(rows) if row.get("motion_stage") == "3"),
        None,
    )
    ...
    for row in rows[:: max(1, len(rows) // 12)]:
        ...
        print(
            ...
            % (
                f("cmd_time_s"),
                row.get("motion_stage"),
                f("world_velocity_x_mps"),
                math.degrees(f("imu_roll_rad")),
                math.degrees(f("imu_pitch_rad")),
                row.get("contact_count"),
                row.get("wbc_shadow_solver_ok"),
                row.get("wbc_full_id_ok"),
