...
def segment_metrics(
    rows: list[dict[str, str]], start: float, end: float, suffix: str
) -> dict[str, object]:
    segment = [row for row in rows if start <= number(row, "cmd_time_s") <= end]
    ok = [
        row
        for row in segment
        if number(row, "wbc_full_srbd_ok") >= 1 and number(row, "wbc_full_id_ok") >= 1
    ]
    result: dict[str, object] = {
        f"{suffix}_rows": len(segment),
        f"{suffix}_event_active_ratio": round(
            sum(number(row, "event_active") >= 1 for row in segment) / len(segment), 4
        )
        if segment
        else math.nan,
        f"{suffix}_wbc_ok_ratio": round(len(ok) / len(segment), 4)
        if segment
        else math.nan,
        ...
        f"{suffix}_ref_yaw_mean_radps": round(
            mean(segment, "event_ref_yaw_rate_radps"), 5
        ),
        ...
        f"{suffix}_max_abs_roll_deg": round(
            math.degrees(maximum_abs(segment, "imu_roll_rad")), 3
        ),
        f"{suffix}_max_abs_pitch_deg": round(
            math.degrees(maximum_abs(segment, "imu_pitch_rad")), 3
        ),
        f"{suffix}_min_base_z_m": round(minimum(segment, "world_base_z_m"), 5),
        f"{suffix}_max_eq_residual": round(
            maximum_abs(segment, "wbc_full_eq_residual"), 6
        ),
    }
    return result
...
def main() -> int:
    ...
    for item in records:
        ...
        if len(spans) == 2:
            result.update(
                {
                    "source_video_start_s": spans[0]["start_s"],
                    "source_video_end_s": spans[0]["end_s"],
                    "target_video_start_s": spans[1]["start_s"],
                    "target_video_end_s": spans[1]["end_s"],
                }
            )
        ...
        if data_path.exists() and len(spans) == 2:
            ...
            source_start, source_end = (
                float(spans[0]["start_s"]) + args.clip_start,
                float(spans[0]["end_s"]) + args.clip_start,
            )
            target_start, target_end = (
                float(spans[1]["start_s"]) + args.clip_start,
                float(spans[1]["end_s"]) + args.clip_start,
            )
    ...
    source_ok = [
        float(row["source_wbc_ok_ratio"])
        for row in rows_out
        if isinstance(row.get("source_wbc_ok_ratio"), (float, int))
        and math.isfinite(float(row["source_wbc_ok_ratio"]))
    ]
    target_ok = [
        float(row["target_wbc_ok_ratio"])
        for row in rows_out
        if isinstance(row.get("target_wbc_ok_ratio"), (float, int))
        and math.isfinite(float(row["target_wbc_ok_ratio"]))
    ]
    report = [
        "# Long-window transition matrix QA",
        "",
        f"- Records: {len(rows_out)}; pass: {sum(row.get('status') == 'pass' for row in rows_out)}.",
        f"- Clip duration: {min(durations):.3f}–{max(durations):.3f} s."
        if durations
        else "- Clip duration: unavailable.",
        f"- WBC full OK ratio, source windows: {min(source_ok):.4f}–{max(source_ok):.4f}."
        if source_ok
        else "- WBC source-window ratio: unavailable.",
        f"- WBC full OK ratio, target windows: {min(target_ok):.4f}–{max(target_ok):.4f}."
        if target_ok
        else "- WBC target-window ratio: unavailable.",
