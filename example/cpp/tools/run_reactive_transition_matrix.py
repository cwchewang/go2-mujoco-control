...
def build_protocol(args: argparse.Namespace, repo: Path) -> dict[str, object]:
    ...
    base_args += [
        ...
        "35",
        "--controller-duration",
        str(args.controller_duration_s),
        "--headless",
    ]
    return {
        ...
        "events": list(EVENTS),
        "source_events": list(
            EVENTS if args.include_terminal_source else NONTERMINAL_EVENTS
        ),
...
def run_one(
    ...
    script_path.write_text(
        make_event_script(source, target, protocol), encoding="utf-8"
    )
    command = [
        "bash",
        str(repo / "example/cpp/scripts/run_trot.sh"),
        str(args.wall_timeout_s),
        relative_experiment,
        *list(protocol["controller_args"]),
        "--event-script",
        str(script_path),
    ]
    ...
    for attempt in range(1, args.max_attempts + 1):
        ...
        attempt_log = (
            runner_log
            if attempt == 1
            else root / "runner_logs" / (f"{run_id}.attempt{attempt}.log")
        )
    ...
    manifest["command"] = [*command, "--domain-id", str(attempts[-1]["domain_id"])]
    ...
    write_json(manifest_path, manifest)
    print(
        f"[{index:03d}] {manifest['status'].upper()} {source} -> {target} "
        f"rc={manifest['returncode']} attempts={len(attempts)}",
        flush=True,
    )
    return manifest
...
def main() -> int:
    ...
    root = (
        args.root
        or repo / "example/cpp/experiments/go2_reactive_transition_matrix_2026-08-20"
    ).resolve()
    ...
    if args.event_start_s < 0 or args.event_duration_s <= 0:
        raise SystemExit(
            "event timing must be non-negative and duration must be positive"
        )
    ...
    for offset, (source, target) in enumerate(selected):
        results.append(
            run_one(
                repo, root, protocol, args.start_index + offset, source, target, args
            )
        )
    write_json(root / "runner_results.json", results)
    failed = [item for item in results if item.get("status") == "failed"]
    print(
        f"Transition matrix runner finished: {len(results) - len(failed)}/{len(results)} subprocesses returned 0.",
        flush=True,
    )
