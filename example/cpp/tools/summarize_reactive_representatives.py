...
def xy(
    rows: list[dict[str, str]], key: str, clip_start: float = 3.5
) -> tuple[list[float], list[float]]:
    pairs = [
        (float(r["cmd_time_s"]) - clip_start, float(r[key]))
        for r in rows
        if r.get(key) not in (None, "") and float(r["cmd_time_s"]) >= clip_start
    ]
    return [p[0] for p in pairs], [p[1] for p in pairs]


def event_span(
    rows: list[dict[str, str]], code: int, clip_start: float = 3.5
) -> tuple[float, float] | None:
    ts = [
        float(r["cmd_time_s"]) - clip_start
        for r in rows
        if int(float(r.get("event_type", "0"))) == code
    ]
...
def main() -> int:
    ...
    runs = {
        name: read_run(args.repo_experiment_root, name)
        for name in [
            "impact_strong_recovery",
            "slip_reference",
            "low_friction_physical",
            "turn_left_long",
            "obstacle_left_physical",
            "obstacle_to_turn_handoff",
        ]
    }
    ...
    for name, rows in runs.items():
        ...
        gt_path = (
            args.repo_experiment_root
            / f"reactive_representative_{name}"
        ...
        if gt_path.exists():
            ...
            obstacle_force = max(
                float(r.get("reactive_obstacle_contact_force_N", "0")) for r in gt
            )
            obstacle_count = max(
                float(r.get("reactive_obstacle_contact_count", "0")) for r in gt
            )
        metrics.append(
            {
                "id": name,
                ...
                "max_abs_yaw_rad": round(
                    max(abs(float(r["imu_yaw_rad"])) for r in rows), 4
                ),
                "max_abs_vy_mps": round(
                    max(abs(float(r["world_velocity_y_mps"])) for r in rows), 4
                ),
    ...
    for rows, label, color in [
        (slip, "slip: actual vx", "tab:orange"),
        (low, "low friction: actual vx", "tab:blue"),
    ]:
    ...
    axes[0].plot(
        x, y, lw=1.0, ls="--", color="tab:green", label="low friction: vx reference"
    )
    ...
    fig, axes = plt.subplots(2, 1, figsize=(12, 7), sharex=True)
    for rows, label, color in [
        (obstacle, "obstacle: measured vy", "tab:green"),
        (turn, "turn: measured vy", "tab:cyan"),
    ]:
        x, y = xy(rows, "world_velocity_y_mps")
        axes[0].plot(x, y, lw=1.2, label=label, color=color)
    x, y = xy(obstacle, "event_ref_vy_mps")
    axes[0].plot(
        x, y, ls="--", lw=1.0, color="darkgreen", label="obstacle vy reference"
    )
    ...
    for rows, label, color in [
        (obstacle, "obstacle: measured yaw", "tab:green"),
        (turn, "turn: measured yaw", "tab:cyan"),
    ]:
        x, y = xy(rows, "imu_yaw_rad")
        axes[1].plot(x, y, lw=1.2, label=label, color=color)
    x, y = xy(obstacle, "event_ref_yaw_rate_radps")
    axes[1].plot(
        x, y, ls="--", lw=1.0, color="darkgreen", label="obstacle yaw-rate ref"
    )
