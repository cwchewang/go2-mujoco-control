...
def paired_differences(active, baseline):
    ...
    for key, tolerance in keys.items():
        diffs = [
            abs(number(active[i], key) - number(baseline[i], key))
            for i in range(count)
            if math.isfinite(number(active[i], key))
            and math.isfinite(number(baseline[i], key))
        ]
        result[key] = {
...
def main():
    ...
    checks = {}
    checks["sensor_only_flag"] = all(
        number(row, "terrain_sensor_only") > 0.5 for row in active
    )
    checks["no_terrain_actuation"] = all(
        number(row, "terrain_actuation") < 0.5 for row in active
    )
    checks["no_hidden_event"] = all(
        number(row, "event_active") < 0.5 and number(row, "event_type") < 0.5
        for row in active
    )
    checks["terrain_enabled"] = all(
        number(row, "terrain_enabled") > 0.5 for row in active
    )
    checks["lidar_provenance"] = all(
    ...
    map_valid_rows = sum(number(row, "terrain_map_valid") > 0.5 for row in active)
    checks["map_telemetry"] = map_valid_rows / len(active) >= 0.80
    checks["finite_map_age"] = all(
        not math.isfinite(number(row, "terrain_map_age_s"))
        or number(row, "terrain_map_age_s") >= 0.0
        for row in active
    )
    ...
    checks["terrain_cap_not_requested"] = all(
        math.isinf(number(row, "terrain_velocity_cap_mps"))
        and number(row, "terrain_velocity_cap_mps") > 0.0
        for row in active
    )
    ...
    checks["no_plan_consumer"] = all(
        number(row, "terrain_plan_consumed") < 0.5
        and number(row, "terrain_gait_target_overrides") < 0.5
        and number(row, "terrain_mpc_plan_consumed") < 0.5
        for row in active
    )
    checks["planner_deadline"] = all(
        math.isfinite(number(row, "terrain_solver_elapsed_us"))
        and number(row, "terrain_solver_elapsed_us") <= 5000.0
        and number(row, "terrain_planner_deadline_misses") < 0.5
        for row in active
    )
    checks["planner_updated"] = (
        max((number(row, "terrain_planner_updates") for row in active), default=0.0)
        > 0.0
    )
    phase1 = {}
    ...
    checks["phase1_quantitative"] = (
        ...
        if args.fixed_3mps
            ...
            and phase1.get("strict_pass") is True
    ...
    contract_path = (
        args.run_dir.parents[4] / "docs" / "research" / "PHASE2_ACCEPTANCE.md"
    )
    holdout_path = (
        args.run_dir.parents[4] / "docs" / "research" / "PHASE2_HOLDOUT_MANIFEST.json"
    )
    analyzer_path = (
        args.run_dir.parents[4] / "example" / "cpp" / "tools" / "analyze_phase2_b0.py"
    )
    ...
    checks["holdout_manifest_hash"] = (
        artifact_manifest.get("phase2_holdout_manifest_sha256") == sha256(holdout_path)
        if holdout_path.is_file()
        else False
    )
    checks["analyzer_hash"] = (
        artifact_manifest.get("phase2_b0_analyzer_sha256") == sha256(analyzer_path)
    ...
    if args.fixed_3mps:
        fixed_text = (
            fixed_output.read_text(encoding="utf-8", errors="replace")
            if fixed_output.is_file()
            else ""
        )
        ...
        checks["fixed_analyzer_hash"] = (
            ...
            if fixed_analyzer_path.is_file()
            else False
        )
    else:
    ...
    if args.baseline:
        ...
        paired_contract = {
            ...
            == baseline_manifest.get("artifacts", {}).get("controller_sha256"),
            "same_simulator_sha256": active_manifest.get("artifacts", {}).get(
                "simulator_sha256"
            )
            == baseline_manifest.get("artifacts", {}).get("simulator_sha256"),
            "same_scene_sha256": active_manifest.get("artifacts", {}).get(
                "scenario_sha256"
            )
            == baseline_manifest.get("artifacts", {}).get("scenario_sha256"),
            "same_profile_sha256": active_manifest.get("profile", {}).get("sha256")
            == baseline_manifest.get("profile", {}).get("sha256"),
        }
    ...
    result = {
        ...
        "terrain_planner_deadline_misses": max(
            (number(row, "terrain_planner_deadline_misses") for row in active),
            default=float("nan"),
        ),
        "checks": checks,
    ...
    result["acceptance_status"] = (
        "PASS" if result["legacy_status_pass"] and all(checks.values()) else "FAIL"
    )
