...
def read_series(
    path: Path, key: str, clip_start: float, duration: float
...
def interpolate(
    path: Path, key: str, clip_start: float, duration: float, frame_times: np.ndarray
) -> np.ndarray:
...
def bounds(*series: np.ndarray, minimum_span: float) -> tuple[float, float]:
    values = np.concatenate(
        [value[np.isfinite(value)] for value in series if value.size]
    )
    if values.size == 0:
...
def render_chart_base(
    ...
    yaw_ref = interpolate(
        data_path, "event_ref_yaw_rate_radps", clip_start, duration, times
    )

    plt.rcParams.update({"font.family": "DejaVu Sans", "font.size": 9})
    fig, (ax_v, ax_yaw) = plt.subplots(
        2,
        1,
        figsize=(6.4, 7.2),
        dpi=100,
    ...
    ax_v.plot(
        times, vx_ref, color="#65e572", linewidth=1.2, linestyle="--", label="ref vx"
    )
    ax_v.plot(times, vy, color="#46d7ff", linewidth=1.2, label="world vy")
    ax_v.plot(
        times, vy_ref, color="#ffbd63", linewidth=1.2, linestyle="--", label="ref vy"
    )
    ax_yaw.plot(times, yaw, color="#ff74c8", linewidth=1.4, label="gyro z")
    ax_yaw.plot(
        times, yaw_ref, color="#f7ef64", linewidth=1.2, linestyle="--", label="ref yaw"
    )
    ...
    fig.suptitle(
        title, x=0.06, ha="left", color="white", fontsize=12, fontweight="bold"
    )
    fig.text(0.06, 0.955, subtitle, color="#a9bdc8", fontsize=8, ha="left")
    fig.text(
        0.06,
        0.015,
        "solid = measured feedback   dashed = controller reference   cursor = current frame",
        color="#9eb0ba",
        fontsize=7,
        ha="left",
    )
    ...
    for span in spans:
        ...
        ax_v.text(
            ...
            ha="center",
            va="top",
            color=COLORS.get(str(span.get("color", "orange")), "#ffae42"),
            fontsize=7,
            fontweight="bold",
            clip_on=True,
        )
...
def make_chart_frames(
    base_path: Path,
    output_dir: Path,
    duration: float,
    geometry: tuple[tuple[int, int], tuple[int, int], tuple[int, int]],
) -> int:
    ...
    for index in range(frame_count):
        ...
        x = int(
            round(
                x_bounds[0]
                + (x_bounds[1] - x_bounds[0]) * min(1.0, max(0.0, t / duration))
            )
        )
        draw.line((x, y_v[0], x, y_yaw[1]), fill="#ff4358", width=3)
        draw.line((x - 1, y_v[0], x - 1, y_yaw[1]), fill="#ffd1d6", width=1)
        draw.rectangle(
            (width - 135, 34, width - 8, 56), fill="#182532", outline="#718896"
        )
...
def render_panel(
    ffmpeg: Path, source: Path, frames: Path, output: Path, duration: float
) -> None:
    command = [
        str(ffmpeg),
        "-y",
        ...
        str(frames / "%06d.png"),
        "-filter_complex",
        "[0:v]fps=20[sim];[1:v]format=yuv420p[data];[sim][data]hstack=inputs=2:shortest=1[v]",
        "-map",
        "[v]",
        "-an",
        "-t",
...
def make_collection(
    ffmpeg: Path, paths: list[Path], output: Path, scratch: Path
) -> None:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="reactive_data_panels_") as temp:
        ...
        for item in records:
            ...
            geometry = render_chart_base(
                data_path,
                str(item["title"]),
                str(item["subtitle"]),
                list(item.get("event_spans", [])),
                float(item["clip_start_s"]),
                duration,
                base,
            )
    ...
    panel_records.sort(
        key=lambda item: [str(x["id"]) for x in records].index(str(item["id"]))
    )
    (args.output_root / "data_panel_manifest.json").write_text(
        json.dumps(panel_records, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    readme = args.output_root / "DATA_PANEL_README.md"
    readme.write_text(
        "# Representative data-panel videos\n\n左侧是原始 MuJoCo 仿真，右侧是逐帧同步面板：实线为测量反馈，虚线为控制器参考，红色游标为当前视频时刻。上图为 vx/vy，下图为 yaw rate；底色和标签对应事件窗口。原始视频仍保留在 `videos/`。\n",
        encoding="utf-8",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="reactive_panel_collection_") as temp:
        make_collection(
            ffmpeg,
            [Path(str(item["video"])) for item in panel_records],
            collection,
            Path(temp),
        )
