...
def max_adjacent_velocity_jump(data: list[dict[str, str]], center: float) -> float:
    ...
    for row in data:
        ...
        if not math.isfinite(state_time) or abs(state_time - center) > 0.40:
            continue
        velocity = (
            number(row, "raw_body_velocity_x_mps"),
        ...
        if previous is not None:
            values.append(
                math.hypot(velocity[0] - previous[0], velocity[1] - previous[1])
            )
        previous = velocity
...
def analyze(path: Path, push_tolerance_s: float) -> dict:
    ...
    sequence_ok = [item["type"] for item in observed] == [
        "none",
        "impact",
        "emergency_stop",
    ]
    source_ok = (
        ...
        and impact.get("source") == 2
        and stop.get("source") == 1
    )
    latency = (
        state_impact - push
        if math.isfinite(state_impact) and math.isfinite(push)
        else math.nan
    )
    stop_delay = (
        state_stop - state_impact
        if math.isfinite(state_stop) and math.isfinite(state_impact)
        else math.nan
    )
    ...
    timing_ok = (
        ...
        and -0.10 <= latency <= push_tolerance_s
        and math.isfinite(stop_delay)
        and 0.35 <= stop_delay <= 0.80
    )
    ...
    result.update(
        {
            ...
            "impact_state_time_s": state_impact,
            "emergency_stop_state_time_s": state_stop,
            "impact_detection_latency_s": latency,
            "emergency_stop_delay_s": stop_delay,
...
def main() -> int:
    ...
    lines = [
        "# Automatic physical-impact acceptance",
        "",
